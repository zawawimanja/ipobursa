...
// Collect all unique stock IDs that appeared in Kumpulan 1 or 2 of any archive file
...
files.forEach(file => {
    ...
    try {
        ...
        const k1 = content.kumpulan_1?.stocks || [];
        const k2 = content.kumpulan_2?.stocks || [];
...
console.log(`Found ${archiveStocks.size} unique stocks across all archive files.`);

// Now let's analyze each one 1-by-1 against the dashboard's filtering rules
console.log('\n========================================================================');
console.log('🔍 INDIVIDUAL AUDIT OF ALL HISTORICAL ARCHIVE STOCKS (1-BY-1)');
console.log('========================================================================');
...
archiveStocks.forEach((val, id) => {
    ...
    if (!ipo) {
        console.log(`❌ ${id} (${val.symbol}): MISSING from data.json entirely!`);
        return;
    }
    ...
    // Audit steps matching sifu-picks filter:
    let status = 'PASSED';
    let reason = '';
    ...
    // Check minimum upside & pullback rule
    if (tp > 0) {
    ...
    console.log(`• ${ipo.symbol || id.toUpperCase()} (${id}) | Price: RM ${curPrice} | TP: RM ${tp} | Upside: ${upside.toFixed(1)}% | Grade: ${grade} | Status: ${status} ${reason ? '-> Reason: ' + reason : ''}`);
});
