...
function getCalibratedTarget(ipo, p) {
    let target = ipo.cincai;
    const isTheme = ["tech", "energy", "consumer"].includes(ipo.ipoDataset);
    
    // 1. High OS retail pump (momentum play)
    if (ipo.os >= 70) {
        target *= p.superHighOsMult;
    } else if (ipo.os >= 40) {
        target *= p.highOsMult;
    }
    
    // 2. High-Growth Sector Theme calibration (only for active stocks)
    if (ipo.os >= 15 || ipo.market === 'main') {
        if (ipo.sector === 'tech') {
            target *= p.techMult;
        } else if (ipo.sector === 'energy') {
            target *= p.energyMult;
        } else if (ipo.sector === 'consumer') {
            target *= p.consumerMult;
        }
    }
    
    // 3. Traditional small-cap discount
    if ((ipo.sector === 'industrial' || ipo.sector === 'construction' || ipo.sector === 'property') && ipo.market === 'ace') {
        if (ipo.os < 15) {
            target *= p.lowOsTradAceDiscount;
        } else {
            target *= p.tradAceDiscount;
        }
    }
    
    // 4. Main market premium (decent institutions + retail backing)
    if (ipo.market === 'main' && ipo.os >= 10) {
        target *= p.mainMktPremium;
    }
    
    return target;
}

function calcAccuracy(dataset, p) {
    let totalError = 0;
    dataset.forEach(ipo => {
        const pred = getCalibratedTarget(ipo, p);
...
console.log('Running Conditional Rules Monte Carlo simulation...');
let bestAcc = 0;
let bestParams = {};
...
for (let i = 0; i < iterations; i++) {
    const p = {
        superHighOsMult: Math.random() * 1.5 + 0.8, // 0.8 to 2.3
        highOsMult: Math.random() * 0.8 + 0.8,      // 0.8 to 1.6
        techMult: Math.random() * 1.5 + 0.8,        // 0.8 to 2.3
        energyMult: Math.random() * 1.2 + 0.8,      // 0.8 to 2.0
        consumerMult: Math.random() * 1.2 + 0.8,    // 0.8 to 2.0
        tradAceDiscount: Math.random() * 0.4 + 0.6, // 0.6 to 1.0
        lowOsTradAceDiscount: Math.random() * 0.4 + 0.5, // 0.5 to 0.9
        mainMktPremium: Math.random() * 0.6 + 0.8    // 0.8 to 1.4
    };
    
    const acc = calcAccuracy(ipoDataset, p);
    if (acc > bestAcc) {
        bestAcc = acc;
        bestParams = p;
    }
...
const results = ipoDataset.map(ipo => {
    const pred = getCalibratedTarget(ipo, bestParams);
...
console.log('\n--- Full 60 IPO Conditional Calibration Table ---');
console.table(results.slice(0, 30));
console.table(results.slice(30, 60));
