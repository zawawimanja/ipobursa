...
function getIpoGrade(ipo) {
    ...
    if (ipo.predictedGrade && effectiveStage < 4) return { grade: ipo.predictedGrade };
    ...
    if (effectiveStage < 5 && os === 0) {
        let score = 0;
        if (isHero) score += 40; else if (isTopTier) score += 30; else if (isMomentum) score += 20;
        if (isTrendingSector) score += 30;
        if (isExpansionFund) score += 20;
        if (isMainMarket) score += 10; else if (isAceMarket) score += 5;
        if (ipo.ofs === true) score -= 15;
        if (pe > 0 && pe < 13.0) score += 15; else if (pe > 0 && pe < 18.0) score += 5; else if (pe > 22.0) score -= 10;
        if (score >= 70) return { grade: 'A' }; if (score >= 40) return { grade: 'B' }; return { grade: 'C' };
    }
...
const validPicks = ipoData.filter(ipo => {
    ...
    const isMatch = ipo.shariah === true && (ipo.stage === 5 || ipo.status === 'Listed') && curPrice > 0 && passesIpoPriceCheck;
...
let scoredPicks = [...validPicks].filter(ipo => {
    ...
    const isMomentumRebound = typeof ipo.dailyChange === 'number' && ipo.dailyChange >= 10.0;
    const isSifuPick = sifuPortfolio.includes((ipo.id || '').toLowerCase()) || sifuPortfolio.includes((ipo.symbol || '').toLowerCase());
    
    // NEW LOGIC: Allow if ATH or Momentum or Sifu Pick
    ...
    } else if (!isNearAth && (typeof ipo.dailyChange !== 'number' || ipo.dailyChange < 10.0)) {
        score -= 100;
    }
    
    // Add momentum breakout bonus
    const isActualAth = highPrice > 0 && curPrice >= (highPrice - 0.005);
    if (isActualAth || (typeof ipo.dailyChange === 'number' && ipo.dailyChange >= 10.0)) {
        score += 80;
    }
    
    const isPortfolio = sifuPortfolio.includes((ipo.id || '').toLowerCase()) || sifuPortfolio.includes((ipo.symbol || '').toLowerCase());
...
top5.forEach((item, index) => {
    const setupName = item.isNearAth ? '🔥 RBS Retest / ATH' : (item.isHealthyDip ? '🟢 Healthy Dip' : '📈 Pullback');
    console.log(`${index+1}. ${item.ipo.symbol || item.ipo.id} (${item.ipo.companyName})`);
    console.log(`   Gred: ${item.grade} | Gaya: ${item.styleName} | Setup: ${setupName}`);
    console.log(`   Harga: RM ${item.ipo.currentPrice.toFixed(3)} | Sifu TP: RM ${item.ipo.targetPrice.toFixed(2)} | Upside: ${item.upside.toFixed(1)}%`);
